...
from PySide6.QtGui import QAction, QColor, QFont, QDrag
...
class NodeTreeWidget(QTreeWidget):
    ...
    def startDrag(self, supportedActions):
        ...
        node_info = {
            "name": node_name,
            "category": category,
            "node_type": self._get_node_type(category)
        }
        
        # 添加输入输出端口信息
        node_info["inputs"] = self._get_node_inputs(node_name)
        node_info["outputs"] = self._get_node_outputs(node_name)
        
        # 将节点信息转换为字符串
        import json
        mime_data.setText(json.dumps(node_info))
    ...
    def _get_node_type(self, category):
        """根据分类获取节点类型"""
        type_map = {
            "设备控制": "device_control",
        ...
        return type_map.get(category, "base")
    
    def _get_node_inputs(self, node_name):
        """获取节点的输入端口"""
        # 根据节点名称返回对应的输入端口
        inputs_map = {
            "SetLockInFreq": ["device_in", "freq_value"],
            "SetLockInPhase": ["device_in", "phase_value"],
            "SetLockInTimeCo": ["device_in", "time_constant"],
            "SetDataSampleRate": ["device_in", "sample_rate"],
            "get_max_slope": ["data_in"],
            "lockin_res": ["data_in"],
            "log_insert_compile": ["data_in"],
            "循环_重复遍历": ["data_in", "iterations"],
            "add_row_data": ["data_in", "row_data"],
            "set_plot": ["data_in"],
            "add_data_line": ["plot_in", "data_in"]
        }
        return inputs_map.get(node_name, [])
    
    def _get_node_outputs(self, node_name):
        """获取节点的输出端口"""
        # 根据节点名称返回对应的输出端口
        outputs_map = {
            "SetLockInFreq": ["device_out"],
            "SetLockInPhase": ["device_out"],
            "SetLockInTimeCo": ["device_out"],
            "SetDataSampleRate": ["device_out"],
            "get_max_slope": ["slope_out"],
            "lockin_res": ["result_out"],
            "log_insert_compile": ["compiled_out"],
            "循环_重复遍历": ["data_out"],
            "add_row_data": ["data_out"],
            "set_plot": ["plot_out"],
            "add_data_line": ["plot_out"]
        }
        return outputs_map.get(node_name, [])
