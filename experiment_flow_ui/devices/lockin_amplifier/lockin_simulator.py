...
class LockinAmplifierSimulator(BaseDevice):
    """仿真锁相放大器类"""

    def __init__(self, device_id: Optional[str] = None, name: str = "LockinAmplifierSimulator"):
        ...
        self.properties = {
            "model": "Simulated Lock-in Amplifier",
    ...
    async def connect(self) -> bool:
        """连接设备"""
        print(f"连接仿真锁相放大器: {self.name}")
        await asyncio.sleep(0.5)
        self.connected = True
        self.status = "online"
        print(f"仿真锁相放大器 {self.name} 连接成功")
        return True

    async def disconnect(self) -> bool:
        """断开设备"""
        print(f"断开仿真锁相放大器: {self.name}")
        await asyncio.sleep(0.2)
        self.connected = False
        self.status = "offline"
        print(f"仿真锁相放大器 {self.name} 断开成功")
        return True
    ...
    async def read(self, **kwargs) -> Any:
        ...
        frequency = kwargs.get("frequency", self.properties.get("frequency", 1000))
        amplitude = kwargs.get("amplitude", self.properties.get("amplitude", 1.0))
        noise = kwargs.get("noise", 0.1)

        data = amplitude * np.sin(2 * np.pi * frequency * t) + noise * np.random.randn(sample_count)
        self.simulated_data = data.tolist()

        print(f"从仿真锁相放大器 {self.name} 读取数据，长度: {len(self.simulated_data)}")
    ...
    async def write(self, data: Any, **kwargs) -> bool:
        ...
        print(f"向仿真锁相放大器 {self.name} 写入数据: {data}")
        return True
    ...
    async def execute_command(self, command: str, **kwargs) -> Any:
        ...
        print(f"在仿真锁相放大器 {self.name} 上执行命令: {command}")
